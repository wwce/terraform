...
resource "google_storage_bucket_object" "config_file_bootstrap" {
  name   = "config/bootstrap.xml"
  source = "bootstrap/bootstrap.xml"
  bucket = "${google_storage_bucket.bootstrap_bucket.name}"
}
resource "google_storage_bucket_object" "config_file_init_cfg" {
  name   = "config/init-cfg.txt"
  source = "bootstrap/init-cfg.txt"
  bucket = "${google_storage_bucket.bootstrap_bucket.name}"
}
resource "google_storage_bucket_object" "content_file_null" {
  name   = "content/null.txt"
  source = "bootstrap/null.txt"
  bucket = "${google_storage_bucket.bootstrap_bucket.name}"
}
resource "google_storage_bucket_object" "bucket-software" {
  name   = "software/null.txt"
  source = "bootstrap/null.txt"
  bucket = "${google_storage_bucket.bootstrap_bucket.name}"
}
resource "google_storage_bucket_object" "bucket-license" {
