...
data "template_file" "handoff-state-file" {
  template = var.handoff_state_filename
}
...
resource "null_resource" "handoff-state-json-app" {
...
resource "null_resource" "gateway-load-balancer-endpoint" {
  provisioner "local-exec" {
    command = "python3 ${path.module}/${var.python_script} create"
  }
  provisioner "local-exec" {
    when = destroy
    command = "python3 ${path.module}/${var.python_script} destroy "
  }
  depends_on = [null_resource.handoff-state-json-app]
}
