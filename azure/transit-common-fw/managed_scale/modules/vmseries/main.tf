...
resource "azurerm_network_interface" "nic0" {
  count                     = length(var.fw_names)
  name                      = "${var.prefix}${element(var.fw_names, count.index)}-nic0"
  location                  = var.location
  resource_group_name       = var.resource_group_name
...
resource "azurerm_network_interface" "nic1" {
  count                     = length(var.fw_names)
  name                      = "${var.prefix}${element(var.fw_names, count.index)}-nic1"
  location                  = var.location
  resource_group_name       = var.resource_group_name
...
resource "azurerm_network_interface" "nic2" {
  count                     = length(var.fw_names)
  name                      = "${var.prefix}${element(var.fw_names, count.index)}-nic2"
  location                  = var.location
  resource_group_name       = var.resource_group_name
  network_security_group_id = azurerm_network_security_group.data.id
  enable_ip_forwarding      = true
...
resource "azurerm_virtual_machine" "vmseries" {
  count                        = length(var.fw_names)
  name                         = "${var.prefix}${element(var.fw_names, count.index)}"
  location                     = var.location
  resource_group_name          = var.resource_group_name
  vm_size                      = var.fw_size
  ...
  storage_os_disk {
    name              = "${var.prefix}${element(var.fw_names, count.index)}-osdisk"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "${var.prefix}${element(var.fw_names, count.index)}"
